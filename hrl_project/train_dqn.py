...
# import clustering
from anomaly import get_cluster_fn
import dqn
import gym
...
tf.flags.DEFINE_boolean('use_intrinsic_timeout', True, 'Whether or not to intrinsically timeout controller agent.')
...
def run(env_name='MountainCar-v0',
    ...
    Args:
        ...
        agent_type: The type RL agent that will be used for training. 'dqn' or 'h_dqn'
    ...
    ###***********Main code************###
    experiment_set = 'num_clusters_' + str(num_clusters) + '_agent_type_'+ agent_type +\
    '_use_extra_travel_penalty_' + str(use_extra_travel_penalty) + '_use_extra_bit_' + \
    str(use_extra_bit) + '_use_controller_dqn_' + str(use_controller_dqn) + \
    '_use_intrinsic_timeout_' + str(use_intrinsic_timeout) + '_use_memory_' + \
    str(use_memory) + '_memory_size_' + str(memory_size) + '_pretrain_controller_' + \
    str(pretrain_controller) + '_run_number_' + str(run_number)
    ...
    for it in range(num_iterations):
        ...
        for train_episode in range(num_train_episodes):
            ...
            while not terminal:
                ...
                if action == 1:
                    env_action = np.random.choice([0,2], 1)[0]
                    ### NOTE!!! ###
                else:
                    env_action = action
        ...
        for eval_episode in range(num_eval_episodes):
            ...
            while not terminal:
                ...
                # Remove the do-nothing action.
                if action == 1:
                    env_action = np.random.choice([0,2], 1)[0] ### NOTE!!! ###
                else:
                    env_action = action
