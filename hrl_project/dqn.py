...
class DqnAgent(object):
    ...
    TARGET_UPDATE = 10 #10000

    def __init__(self, sess=None, learning_rate=0.00025, state_dims=[], num_actions=0,
        epsilon_start=1.0, epsilon_end=0.02, epsilon_decay_steps=50000, replay_memory_init_size=None,
        target_update=None):
        ...
        self.final_epsilon = epsilon_end
    ...
    def _q_network(self, state):
        ...
        layer1 = tf.contrib.layers.fully_connected(state, 50, activation_fn=tf.nn.relu)
        ...
        q_values = tf.contrib.layers.fully_connected(layer2, self._num_actions, activation_fn=None)
    ...
    def sample(self, state):
        ...
        self.epsilon = self.REPLAY_MEMORY_INIT_SIZE / (self.REPLAY_MEMORY_INIT_SIZE + self._current_time_step)
    ...
    def update(self):
        ...
        print ""
        '''
        actions_temp = np.zeros((len(actions),2))
        actions_temp[:,0] = np.arange(len(actions))
        actions_temp[:,1] = actions
